...
def main():
    ...
    for key in sorted(list(losses_dendritic.keys())):
        ...
        try:
            axs[i].plot(ts_dendritic[key]*key[1], losses_dendritic[key], 
                    label="DB", color='orange')
        except:
            continue
        try:
            axs[i].plot(ts_somatic[key]*key[1], losses_somatic[key], 
                    label="SB", color='darkolivegreen')
        except:
            continue
    ...
    plt.tight_layout()
    plt.savefig("scancompare_timstep_loss_poprate_2d.svg")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("scancompare_timstep_loss_poprate_compact_2d.svg")
    ...
    plt.tight_layout()
    plt.savefig("scancompare_timstep_loss_poprate_spikes_2d.svg")
    ...
    plt.savefig("scancompare_timstep_weights_2d.svg", dpi=500)
