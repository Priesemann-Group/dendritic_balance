...
def main():
    ARGS = sys.argv
    if (len(ARGS) <= 4):
        print("Please provide four folders as argument (1. dendritic analytic, 2. dendritic simultaneous, 3. dendritic delay, 4. somatic).")
        return
    
    folder_dendritic = ARGS[1] 
    folder_dendritic_sim = ARGS[2] 
    folder_dendritic_del = ARGS[3] 
    folder_somatic = ARGS[4] 
    
    names = ["DB", "DB simultaneous", "DB delayed", "SB"]
    folders = [folder_dendritic, folder_dendritic_sim, folder_dendritic_del, folder_somatic]
    ...
    axs = [fig.add_subplot(gs[0, :]), fig.add_subplot(gs[1, 0]),
    ...
    plt.tight_layout()
    plt.savefig("../../plots/mnist_compare_all.svg", dpi=500)
