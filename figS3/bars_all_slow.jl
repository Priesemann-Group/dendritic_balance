include("../src/all_weights_slow/settings.jl")
include("../src/all_weights_slow/main.jl")
...
function main_bars(plotflag, s, corr, noise, id)
    ...
    set!(s, "learningRateFeedForward", 1.0e-7)
    set!(s, "learningRateDecoder", 5.0e-5)
    set!(s, "learningRateInhibitoryRecurrent", 5.0e-5)
    set!(s, "learningRateHomeostaticBias", 1.0e-2)
    set!(s, "learningRateSigma", 7.0e-8)
    set!(s, "learningRateFeedForwardBar", 5.0e-5)
    ...
    return main("S3_bars_all_weights_slow", plotflag, s, nSteps, inputs, test_inputs, test_times)
end
