...
def main():
    ...
    for key in sorted(list(losses_dendritic.keys())):
        ...
        try:
            axs[i].plot(ts_dendritic[key], losses_dendritic[key], 
                    label="DB", color='darkolivegreen')
        except:
            continue
        try:
            axs[i].plot(ts_somatic[key], losses_somatic[key], 
                    label="SB", color='orange')
        except:
            continue
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("scancompare_loss_poprate.svg")
    ...
    plt.tight_layout()
    plt.savefig("scancompare_loss_poprate_compact.svg")
    ...
    plt.tight_layout()
    plt.savefig("scancompare_weights.svg", dpi=500)
