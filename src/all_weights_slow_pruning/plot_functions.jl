...
function plot_snapshot(net::Net, snapshot::Snapshot, folder::String, s::Dict{String,Any})
    ...
    plot_xz_weights(snapshot.xz_weights_bar, folder * "snapshot$ap-$t/xz_weights_bar", s)
    plot_zz_weights(snapshot.zz_weights, snapshot.xz_weights, folder * "snapshot$ap-$t/zz_weights", s)
    w = get_inhibition_weights(snapshot.decoder_weights,snapshot.xz_weights,  1.0)
    plot_total_inhibition(w, folder * "snapshot$ap-$t/zz_weights_perfect_inhibition")
    ...
    plot_reconstruction_comparison(snapshot.reconstructions, -snapshot.reconstructions_net[:,:,1], folder * "snapshot$ap-$t/", s)
...
function plot_reconstruction_comparison(recs::Array{Float64}, xs::Array{Float64},
    ...
    folder = name * "reconstructions/"
    mkpath(folder)
    tmax = min(Int(floor(20*s["presentationLength"]/s["snapshotLogInterval"])),size(recs,1))
    vars = mean((recs - xs).^2, dims=1)
    inds1 = sortperm(reshape(vars,:),lt=(>))
    inds2 = sortperm(reshape(mean(xs, dims=1),:),lt=(>))
    if s["n_z"] > 4
        inds = cat(inds1[1:4], inds1[end-3:end], inds2[1:4], dims=1)
    else
        inds = 1:s["n_z"]
    end
...
function plot_spikes(spikes::Array{Bool}, name::String, s::Dict{String,Any})
    ...
    for n in 1:size(spikes)[2]
        sub = []
        last_t = 1
        for t in 1:min(Int(floor(20*s["presentationLength"]/s["snapshotLogInterval"])),size(spikes)[1])
            if spikes[t,n]
...
function plot_total_inhibition(weights::Array{Float64}, name::String)
    PyPlot.clf()
...
function plot_zz_weights(weights::Array{Float64}, F::Array{Float64}, name::String, s::Dict{String,Any})
    m = size(weights)[1]
    for j in 1:m
        plot_xz_weights(Matrix(weights[j,:,:]'), name * "_$j", s)
        plot_xz_weights_inverted(Matrix(weights[j,:,:]'), name * "_r_$j", s)
        plot_xz_weights(-Matrix(weights[j,:,:]') ./ F[j,:]', name * "_$(j)_D", s)
    end
    plot_total_inhibition(reshape(sum(weights, dims=2), m, :), name * "_total_inhibition")
end
...
function plot_xz_weights_inverted(weights::Array{Float64}, name::String, s::Dict{String,Any})
    splitPosNeg = s["splitPosNegInput"]::Bool
    ...
    img = separate_subimages(img, sidelength, 1, inverted=true)
    PyPlot.clf()
    PyPlot.figure(figsize=(5.0,4.0))
